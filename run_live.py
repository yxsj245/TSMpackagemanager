...
print(Fore.YELLOW +'程序版本3.0-live')
# 环境初始化
# 初始配置文件json
config = load_config()
if config is None:
...
if choice == "1":
    ...
    if xuanze == '1':
        ...
        panduan = fetch_json_from_url('https://pub-46d21cac9c7d44b79d73abfeb727999f.r2.dev/TSM%E5%AE%89%E8%A3%85%E5%99%A8/down.json')
        if panduan['version'] != version_downconfig:
            print(Fore.RED + 'ERROR:云端配置文件不兼容当前运行程序,请下载最新版程序')
            ...
            download_file_with_progress(fileurl, save_path)
            print(Fore.BLUE + 'INFO:下载配置文件成功,请重新运行程序')
elif choice == "9":
    fileurl = down_5
    save_path = os.path.join(filepath, "VC_redist.x64.exe")
    download_file_with_progress(fileurl, save_path)
    ...
    print(Style.BRIGHT + Fore.CYAN + "温馨提示：请前往github确认下载的具体版本号;汉化没有历史版本，若您使用最新版汉化可能会存在部分没有汉化的情况;若是首次安装，请先使用一键安装安装加载器后再使用此功能")
    print('=====')
    print(Fore.GREEN + "1. 直接下载(默认)\n"
                       "2. 访问GitHub确认版本号")
    print('=====')
    xuanze = input("请输入选项数字: ")
    if xuanze == '2':
        ...
        if jsondata['Downloadsource'] == 'github'or jsondata['Downloadsource'] == 'cloudflare':
            ...
            url = f"https://github.com/XeTrinityz/ThatSkyMod/releases/download/{version}/TSM.zip"
        else:
            print(Fore.BLUE + "INFO:正在通过GitHub中国代理加速地址下载")
            url = f"https://ghp.ci/https://github.com/XeTrinityz/ThatSkyMod/releases/download/{version}/TSM.zip"
        # 下载TSM.zip
        save_path = os.path.join(filepath, "TSM.zip")
        ...
        print(Fore.BLUE + "INFO:开始解压文件")
        unzip_file(create_full_path(base_path, file_name), game_path_mod)
        print(Fore.BLUE + "INFO:解压完毕，请在steam中启动游戏")
elif choice == "12":
    ...
    print(Fore.BLUE + 'INFO:重置完毕')
elif choice == "13":
    print('=====')
    print(Fore.GREEN + '1.禁用 TSM\n'
                       '2.启用 TSM')
    print('=====')
    xuanze = input("请输入选项数字: ")
    if xuanze == '1':
